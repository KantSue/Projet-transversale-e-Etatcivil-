...
def simuler_paiement(id_type_acte, numero_tel, nb_exemplaires=1):
    montant_unitaire = MONTANTS.get(int(id_type_acte), 1000)
    montant          = montant_unitaire * int(nb_exemplaires)
    reference        = f"TXN-{uuid.uuid4().hex[:12].upper()}"
    succes           = True  # simulation toujours succès
